...
Give each side a context value naming it. This is what `compare` pivots on, and
...
>   mew run bench_alpha.py --append -o results.jsonl
>   mew run bench_beta.py  --append -o results.jsonl
> done
...
```console
$ for i in 1 2 3; do
>   .venv-gil/bin/mew run bench_x.py --append -o results.jsonl
>   .venv-ft/bin/mew  run bench_x.py --append -o results.jsonl
> done
```
...
```console
$ mew compare results.jsonl --by context.build
```
...
$ mew compare results.jsonl --by context.engine --metric memory.allocations_per_iteration
```
